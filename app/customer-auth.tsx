  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isSignUp) {
        ...
        // Load their display name from auth
      ...
      // Ensure a session ID exists for order tracking
        ...
        >
          ...
          <YStack alignItems="center" space="$3" marginBottom="$8">
            <YStack
              ...
              borderWidth={2} borderColor="rgba(245,196,0,0.4)"
            >
          ...
          <YStack space="$4" width="100%">
            ...
            <YStack space="$1">
              ...
              >
                ...
                <TextInput
                  ...
                  style={[styles.input, { flex: 1 }, Platform.OS === 'web' ? ({ outlineStyle: 'none' } as any) : {}]}
                />
                <Pressable onPress={() => setShowPassword((v) => !v)} hitSlop={8}>
                  {showPassword ? <EyeOff size={18} color="$color9" /> : <Eye size={18} color="$color9" />}
                </Pressable>
              </XStack>
            </YStack>
            ...
            {/* Error */}
            {!!error && (
              <YStack backgroundColor="$red3" borderRadius={10} paddingHorizontal="$4" paddingVertical="$3" borderWidth={1} borderColor="$red6">
                <SizableText size="$3" color="$red10">{error}</SizableText>
              </YStack>
            )}
            ...
            {isSignUp && (
              <YStack space="$2">
                <XStack space="$3">
                  ...
                  <Pressable onPress={() => router.push('/privacy-policy')} style={({ pressed }) => [styles.linkBtn, pressed && { opacity: 0.6 }]}>
                    <SizableText size="$2" fontWeight="700" color="$yellow10" textDecorationLine="underline">Privacy Policy</SizableText>
                  </Pressable>
                </XStack>
                <Pressable onPress={() => setAgreedToTerms((v) => !v)} style={styles.termsRow}>
                  <YStack
                    width={22} height={22} borderRadius={5}
                    borderWidth={2}
                    borderColor={agreedToTerms ? '#d97706' : 'rgba(255,255,255,0.35)'}
                    backgroundColor={agreedToTerms ? '#d97706' : 'transparent'}
                    alignItems="center" justifyContent="center"
                    flexShrink={0}
                  >
                    {agreedToTerms && (
            ...
            <XStack justifyContent="center" space="$1" marginTop="$2">
              ...
              <Pressable onPress={switchMode}>
                <SizableText size="$3" fontWeight="700" color="$yellow9">
