...
function BGCheckCard({ check }: { check: BackgroundCheck }) {
            ...
            <YStack gap="$1">
              ...
              >
                ...
                <TextInput
                  value={ref}
                  onChangeText={setRef}
                  placeholder="e.g. CHKR-12345 or Sterling case ID"
            ...
            <XStack gap="$2">
              ...
              >
                {resolving
                  ...
                  : <><CheckCircle size={13} color="white" /><SizableText size="$2" fontWeight="700" color="white"> Approve</SizableText></>
                }
              </Pressable>
            </XStack>
          </YStack>
...
export default function AdminBGCheckPanel() {
  const { data: all = [] } = useAllBackgroundChecks();
  const actionable = all.filter((c) => c.status === 'pending' || c.status === 'in_review');
  ...
  return (
    <YStack gap="$3">
      <XStack alignItems="center" gap="$2">
        ...
        >
          <SizableText size="$1" color="white" fontWeight="800">{actionable.length}</SizableText>
        </YStack>
      </XStack>
      <SizableText size="$2" color="$color9">
        Run each check via your screening service, then update the status here.
      </SizableText>
      {actionable.map((c) => <BGCheckCard key={c.id} check={c} />)}
    </YStack>
  );
}
